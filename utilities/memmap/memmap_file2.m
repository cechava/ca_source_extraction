function data = memmap_file2(filename,sframe,num2read,chunksize,order)
...
% Read witih tiffread?
%tic(); [movie, metaMovie] = tiffRead(filename); toc()
...
if isempty(chunksize)
    ...
    Yt = imread(filename,'Index',1:order:(order*nSlices),'Info',info);
    %sizY = [size(Yt),numFrames];
    sizY = [256, 1024, 20, 350];
    if length(sizY) == 3
